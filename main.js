...
minus.addEventListener("click", () => {
    ...
    document.getElementById("adult").innerText = counter;
})
    ...
    if (!arr.includes(srch.value)) { alert("not found") } else {
        ...
        sort.addEventListener("change", () => {
            ...
            if (sort.value === "descending") {
                sorted = filtereD.sort((p1, p2) => (p1.price < p2.price) ? 1 : (p1.price > p2.price) ? -1 : 0)
                hotel(sorted);
            } else if (sort.value === "ascending") {
                sorted = filtereD.sort((p1, p2) => (p1.price < p2.price) ? -1 : (p1.price > p2.price) ? 1 : 0)
                hotel(sorted);
            } else if (sort.value === "") { hotel(filtereD) }


        });

        filter.addEventListener("change", () => {
            let filtered = filtereD.filter((el) => {
                if (el.ratings <= filter.value) { return true } else { return false }
            })

            hotel(filtered);
            if (filter.value === "") { hotel(filtereD) }
        })
    }
})
